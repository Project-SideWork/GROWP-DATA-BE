...
@respx.mock
async def test_project_handler_fetches_each_id(caplog: pytest.LogCaptureFixture) -> None:
    caplog.set_level(logging.INFO, logger="app.clients.recruitment")
    ...
    assert "Backend applicant request started entity_id=10" in caplog.text
    assert "Backend applicant request succeeded entity_id=20 status_code=200" in caplog.text
...
@respx.mock
async def test_client_fetches_club_applicants() -> None:
    request = respx.get("http://localhost:8080/api/v1/analytics/clubs/7/applicants").mock(
        return_value=httpx.Response(200, json={"data": {"clubId": 7}})
    )
    settings = Settings(delivery_max_attempts=1)

    async with httpx.AsyncClient(base_url=str(settings.backend_base_url)) as http_client:
        client = RecruitmentDataClient(http_client, settings)
        result = await client.fetch_clubs([7])

    assert request.call_count == 1
    assert result == [{"data": {"clubId": 7}}]
